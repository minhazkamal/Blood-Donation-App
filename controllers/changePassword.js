            ...
            .then(result => {
                if (result[0].provider !== 'self') {
                    ...
                    db.NotificationUpdateDynamically(req, res)
                        .then(result => {
                            res.render('changePassword.ejs', { is_changable, navbar: req.session.navbar_info, notifications: req.session.notifications });
                        })
...
body('password').custom((value, { req }) => {
    ...
    if (!errors.isEmpty()) {
        ...
        db.NotificationUpdateDynamically(req, res)
            .then(result => {
                res.render('changePassword', { alert, is_changable: req.session.is_password_changable, navbar: req.session.navbar_info, notifications: req.session.notifications });
            })
