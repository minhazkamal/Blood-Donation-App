...
router.get('/:encrypted_id', function (req, res) {
    ...
    let user_id = cryptr.decrypt(encrypted_id);
    ...
    if (req.session.email) {
        db.getuserinfobyid(user_id)
            .then(result => {

                eligibility_parameters.username = result[0].first_name;
                ...
                if (result[0].eligibility_test == 'yes') {
                    db.getEligibilityReportById(user_id)
                        .then(result => {
