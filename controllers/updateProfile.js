        ...
        upload(req, res, err => {
            if (err) {
                db.NotificationUpdateDynamically(req, res)
                                .then(result => {
                                    res.render('updateProfile', { alert: [{ msg: err.message }], navbar: req.session.navbar_info, notifications: req.session.notifications });
                                })
...
router.post('/', validator, [
    ...
    function (req, res) {
        ...
        if (!errors.isEmpty()) {
            ...
            db.NotificationUpdateDynamically(req, res)
                                .then(result => {
                                    res.render('updateProfile', { alert, user: req.session.temp_user, divisions: req.session.div_results, navbar: req.session.navbar_info, notifications: req.session.notifications });
                                })
